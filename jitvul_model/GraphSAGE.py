...
class CodejitGraphSAGE(torch.nn.Module):
    def __init__(self, in_channels, hidden_channels, num_relations=4, dropout=0.1, num_of_layers=2, graph_readout_func="add"):
        ...
        for i in range(self.num_layers):
            if i == 0:
                exec(
                    'self.conv_{} = GraphSAGE(in_channels, hidden_channels,num_layers=self.num_layers, dropout = dropout)'.format(i))
            else:
                exec(
                    'self.conv_{} = GraphSAGE(hidden_channels, hidden_channels,num_layers=self.num_layers, dropout = dropout)'.format(i))
